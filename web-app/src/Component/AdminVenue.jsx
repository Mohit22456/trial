...
export default function AdminVenue(){
  ...
  const loadAllData=async ()=>{
    try{
        const response =await axios.get('http://localhost:8080/admin/venue')
    ...
    try{
      console.log(venueId)
      const response = await axios.delete(`http://localhost:8080/admin/deletevenue/${venueId}`)
  ...
  const editVenue = async (venueId) =>{
    ...
    try {
      const response = await axios.post(`http://localhost:8080/admin/updatevenue/${venueId}`, body)
      loadAllData();
      return response.data
    }catch(error){
      console.error('Error fetching data: ',error);
